...
const TransactionsList = ({ transactions }) => 
    transactions.map((t, i) => (
        <div key={`item-${i}`} className='details-item'>
            <div className='title h3'>
                <Translate id='login.details.forContract' />: <a href={`${process.env.EXPLORER_URL || 'http://192.168.80.123:80'}/accounts/${t.signerId}`} target='_blank' rel="noopener noreferrer" className='color-blue'>@{t.signerId}</a>
            </div>
            {false &&  <ActionsList 
                transaction={t} 
                actions={t.actions}
